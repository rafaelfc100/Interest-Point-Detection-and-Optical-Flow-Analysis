...
import numpy as np
#import os
#os.makedirs(r"C:\Users\rafae\OneDrive\Escritorio\Vision\Tarea3\frames", exist_ok=True)


feature_params = dict(
    maxCorners=500, #puntos máximos a detectar
    qualityLevel=0.01, # la calidad de las esquinas
    minDistance=10, # distancisón mínima entre dos puntos
    blockSize=7
)

lk_params = dict(
    winSize=(15, 15), # venatana de búsqueda
    maxLevel=3, # pirámide (multi-escala)
    criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 10, 0.03) # cuándo parar
)
...
ALPHA              = 0.5   # ema suavizado
CLUSTER_DISTANCE   = 40
MIN_POINTS_CLUSTER = 3
MAX_OBJECTS        = 3


COLOR_TEXT_MAIN   = (255, 0, 255)    # magenta
COLOR_TEXT_OBJ    = (0, 255, 255)    # cyan
COLOR_POINTS      = (0, 140, 255)    # naranja en BGR
COLOR_ARROWS      = (0, 180, 255)    # naranja
COLOR_BBOX_PREV   = (0, 0, 255)      # rojo
COLOR_BBOX_CURR   = (255, 0, 0)      # azul
COLOR_ROI         = (255, 255, 0)    # cian
COLOR_ROI         = (255, 255, 0)
...
def cluster_points_kmeans(points, k=MAX_OBJECTS):
    ...
    for ki in range(1, min(k + 1, len(points) + 1)):
        ...
        ret, labels, centers = cv2.kmeans(
            points_f, ki, None, criteria, 10, cv2.KMEANS_PP_CENTERS
        )
        inertia = 0.0
        for ci in range(ki):
    ...
    _, labels, _ = cv2.kmeans(
        points_f, final_k, None, criteria, 10, cv2.KMEANS_PP_CENTERS
    )
...
def draw_hud(output, frame_count, max_frames, velocity_threshold, n_objects, n_moving):
    ...
    lines = [
        ...
        #(f"Pts en movimiento: {n_moving}",              COLOR_TEXT_MAIN, (10, 50)),
        #(f"Objetos detectados: {n_objects}",            COLOR_TEXT_OBJ,  (10, 100)),
    ]
...
# imagen blanca
def save_accumulative_mask(mask_vectors, frame_shape, output_path="mascara_flujo.png"):
    if not mask_vectors:
        print("No hay vectores para la máscara.")
        return

    h, w = frame_shape[:2]
    mask_img = np.full((h, w, 3), 255, dtype=np.uint8)
    step = max(1, len(mask_vectors) // MAX_MASK_VECTORS)
    selected = mask_vectors[::step][:MAX_MASK_VECTORS]

    COLOR_PURPLE = (180, 0, 180)   # morado en BGR

    ARROW_SCALE = 12   # escalar para que se multiplique y se vean las flechas en la imagen blanca

    for (pt_old, pt_new) in selected:
        x0, y0 = int(pt_old[0]), int(pt_old[1])
        dx = (pt_new[0] - pt_old[0]) * ARROW_SCALE
        dy = (pt_new[1] - pt_old[1]) * ARROW_SCALE
        x1 = int(x0 + dx)
        y1 = int(y0 + dy)
        # Clamp dentro de la imagen
        x1 = np.clip(x1, 0, w - 1)
        y1 = np.clip(y1, 0, h - 1)
        cv2.arrowedLine(mask_img, (x0, y0), (x1, y1),
                        COLOR_PURPLE, 2, tipLength=0.3)
        cv2.circle(mask_img, (x0, y0), 4, COLOR_PURPLE, -1)

    cv2.imwrite(output_path, mask_img)
    print(f"Máscara guardada en: {output_path}  ({len(selected)} vectores)")
    return mask_img



cap = cv2.VideoCapture(r"C:\Users\rafae\OneDrive\Escritorio\Vision\clase\match\videoPieza.mov")
...
while cap.isOpened() and frame_count < max_frames:
    ...
    p1_lk, st, _ = cv2.calcOpticalFlowPyrLK(old_gray, frame_gray, p0_lk, None, **lk_params) # p1_lk es la dirección del flujo
    ...
    p1_valid[:, 0] += x;  p1_valid[:, 1] += y
    p0_valid[:, 0] += x;  p0_valid[:, 1] += y
    ...
    if frame_count % 5 == 0:
        for old_pt, new_pt in zip(moving_points_prev, moving_points_current):
            mask_vectors.append((old_pt.copy(), new_pt.copy()))
    ...
    if 1650 <= frame_count <= 1700:
        save_path = rf"C:\Users\rafae\OneDrive\Escritorio\Vision\Tarea3\frames\frame_{frame_count:04d}.png"
...
print(f"\nTotal vectores acumulados: {len(mask_vectors)}")
mask_img = save_accumulative_mask(
    mask_vectors,
    frame_shape,
    output_path=r"C:\Users\rafae\OneDrive\Escritorio\Vision\Tarea3\mascara_flujo.png"
)

if mask_img is not None:
    cv2.imshow("Mascara acumulativa de flujo optico", mask_img)
    cv2.waitKey(0)
